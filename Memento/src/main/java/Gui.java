...
public class Gui extends Application {
    ...
    public void HistoryWindow() {
        ...
        historyListView.setOnMouseClicked(event -> {
            ...
            if (selected != null) {
                controller.restoreStateFromMetadata(selected);
                updateGui();
            }
        });

        Scene historyScene = new Scene(historyListView, 300, 400);
        historyStage.setScene(historyScene);
        historyStage.show();
    }

    public void addHistoryToView(IMemento memento) {
        if (historyListView != null) {
            historyListView.getItems().add(memento);
        }
    }
